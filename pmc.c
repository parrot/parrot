...
PMC*
do_clone(Parrot_Interp interpreter, PMC* src)
{
    return (PMC*) do_traverse(interpreter, src, TRAV_DO_CLONE);
}

opcode_t*
do_freeze(Parrot_Interp interpreter, PMC* src)
{
    return (opcode_t*) do_traverse(interpreter, src, TRAV_DO_FREEZE);
}

STRING*
do_dump(Parrot_Interp interpreter, PMC* src)
{
    return (STRING*) do_traverse(interpreter, src, TRAV_DO_DUMP);
}

PMC*
do_thaw(Parrot_Interp interpreter, opcode_t*image)
{
    return (PMC*) do_traverse(interpreter, image, TRAV_DO_THAW);
}
...
static STRING*
pmc_2_key(Parrot_Interp interpreter, PMC *pmc, STRING *key)
{
    if (!key) {
...
void*
do_traverse(Parrot_Interp interpreter, void* arg, traverse_enum_t what)
{
    traverse_info info;
    PMC *cur;
