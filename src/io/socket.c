...
static void io_socket_adv_position(PARROT_INTERP,
    ARGMOD_NULLOK(const PMC *handle),
    const size_t offset)
        FUNC_MODIFIES(*handle);

static INTVAL io_socket_close(PARROT_INTERP, ARGMOD(const PMC *handle))
        __attribute__nonnull__(1)
        __attribute__nonnull__(2)
        FUNC_MODIFIES(*handle);

static INTVAL io_socket_flush(PARROT_INTERP,
...
static INTVAL io_socket_is_open(PARROT_INTERP, ARGMOD(const PMC *handle))
        __attribute__nonnull__(1)
        __attribute__nonnull__(2)
        FUNC_MODIFIES(*handle);
...
static PIOOFF_T io_socket_tell(PARROT_INTERP, ARGMOD(const PMC *handle))
        __attribute__nonnull__(1)
        __attribute__nonnull__(2)
        FUNC_MODIFIES(*handle);
...
=item C<static INTVAL io_socket_read_b(PARROT_INTERP, const PMC *handle, const
...
static INTVAL
io_socket_read_b(PARROT_INTERP, ARGMOD(const PMC *handle), ARGOUT(const char *buffer),
                 const size_t byte_length)
{
...
=item C<static INTVAL io_socket_write_b(PARROT_INTERP, const PMC *handle, const
char *buffer, const size_t byte_length)>
...
static INTVAL
io_socket_write_b(PARROT_INTERP, ARGMOD(const PMC *handle), ARGIN(const char *buffer),
                  const size_t byte_length)
{
...
=item C<static INTVAL io_socket_flush(PARROT_INTERP, const PMC *handle)>
...
static INTVAL
io_socket_flush(SHIM_INTERP, ARGIN_NULLOK(const PMC *handle))
{
...
=item C<static void io_socket_set_eof(PARROT_INTERP, const PMC *handle, const
INTVAL is_eof)>
...
static void
io_socket_set_eof(SHIM_INTERP, ARGIN_NULLOK(const PMC *handle), SHIM(const INTVAL is_eof))
{
...
static PIOOFF_T
io_socket_tell(PARROT_INTERP, ARGMOD(const PMC *handle))
{
...
=item C<static PIOOFF_T io_socket_seek(PARROT_INTERP, const PMC *handle, const
...
static PIOOFF_T
io_socket_seek(PARROT_INTERP, ARGMOD(const PMC *handle), SHIM(const PIOOFF_T offset),
...
=item C<static void io_socket_adv_position(PARROT_INTERP, const PMC *handle,
const size_t offset)>

Do nothing. Sockets don't keep track of position.

=item C<static void io_socket_set_position(PARROT_INTERP, const PMC *handle,
const PIOOFF_T pos)>
...
static void
io_socket_adv_position(SHIM_INTERP, ARGMOD_NULLOK(const PMC *handle),
...
static void
io_socket_set_position(SHIM_INTERP, ARGMOD_NULLOK(const PMC *handle),
...
=item C<static INTVAL io_socket_open(PARROT_INTERP, const PMC *handle, const
...
static INTVAL
io_socket_open(PARROT_INTERP, ARGMOD(const PMC *handle), SHIM(const STRING *path),
...
static INTVAL
io_socket_is_open(PARROT_INTERP, ARGMOD(const PMC *handle))
{
...
=item C<static INTVAL io_socket_close(PARROT_INTERP, const PMC *handle)>
...
static INTVAL
io_socket_close(PARROT_INTERP, ARGMOD(const PMC *handle))
{
...
=item C<static void io_socket_set_flags(PARROT_INTERP, const PMC *handle, const
INTVAL flags)>
...
static void
io_socket_set_flags(SHIM_INTERP, ARGIN_NULLOK(const PMC *handle), SHIM(const INTVAL flags))
{
