        ...
        size_t object_size)
{
    ...
    new_object_size =
            (object_size + sizeof(void *) - 1) & ~(sizeof(void *) - 1);
        ...
        size_t object_size, struct Small_Object_Pool *pool)
{
    ...
    if (new_object_size > 64) {
        internal_exception(ALLOCATION_ERROR,
                "Error: Cannot allocate sized small_object pools "
                "of more than 64 bytes\n");
    }
    ...
    if (sized_pools[idx] == NULL) {
        ...
        internal_exception(ALLOCATION_ERROR,
                "Error: Tried to set a small-object pool when one "
