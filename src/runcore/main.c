...
parrot_hash_oplib(PARROT_INTERP, ARGIN(op_lib_t *lib))
{
    ...
    for (i = 0; i < lib->op_count; i++) {
        ...
        Parrot_hsh_put(interp, interp->op_hash, (void *)op->full_name,
                                                 (void *)op);

        if (!Parrot_hsh_exists(interp, interp->op_hash, (void *)op->name))
            Parrot_hsh_put(interp, interp->op_hash, (void *)op->name,
                                                     (void *)op);
    }
}
