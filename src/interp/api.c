...
=item C<Parrot_Interp Parrot_interp_make_interpreter(Interp *parent, INTVAL flags)>
...
=item C<Parrot_Interp Parrot_interp_allocate_interpreter(Interp *parent, INTVAL flags)>
...
=item C<Parrot_Interp Parrot_interp_initialize_interpreter(PARROT_INTERP, Parrot_GC_Init_Args
*args)>
...
=item C<void Parrot_interp_really_destroy(PARROT_INTERP, int exit_code, void *arg)>
...
=item C<void Parrot_interp_register_nci_method(PARROT_INTERP, const int type, void *func,
...
=item C<void Parrot_interp_register_native_pcc_method_in_ns(PARROT_INTERP, const int type,
...
=item C<void Parrot_interp_mark_method_writes(PARROT_INTERP, int type, const char
*name)>
...
=item C<void Parrot_interp_set_compiler(PARROT_INTERP, STRING *type, PMC *compiler)>
...
=item C<Parrot_PMC Parrot_interp_compile_string(PARROT_INTERP, Parrot_String type,
...
=item C<void Parrot_interp_set_executable_name(PARROT_INTERP, STRING * const name)>
