...
#include "pmc/pmc_callcontextfromc.h"
...
typedef struct pcc_funcs_ptr {
    ...
    string_func_t       string_constant; /* Name of the parameter */
} pcc_funcs_ptr;
...
#define ASSERT_ARGS_collect_named_params __attribute__unused__ int _ASSERT_ARGS_CHECK = (\
       PARROT_ASSERT_ARG(interp) \
        ...
        ARGIN(PMC *raw_sig), ARGIN(opcode_t *raw_args))
{
    ...
    SETATTR_CallContext_raw_args(interp, call_object, raw_args + 2);
    SETATTR_CallContext_from_context(interp, call_object, ctx);

    //SETATTR_CallContext_arg_flags(interp, call_object, raw_sig);
    VTABLE_set_pmc(interp, call_object, raw_sig);
        ...
        ARGMOD(va_list *args))
{
    ...
    signature = Parrot_pmc_new(interp, enum_class_CallContextFromC);
        ...
        Errors_classes direction)
{
    ...
    while (param_index < param_count) {
        ...
        if (param_flags & PARROT_ARG_SLURPY_ARRAY) {
            ...
            if (!(param_flags & PARROT_ARG_NAME)) {
                ...
                while (VTABLE_elements(interp, call_object)) {
                    VTABLE_set_pmc_keyed_int(interp, collect_positional, j++,
                        VTABLE_shift_pmc(interp, call_object));
                }
        ...
        if (VTABLE_elements(interp, call_object)) {
            ...
            switch (PARROT_ARG_TYPE_MASK_MASK(param_flags)) {
              ...
              case PARROT_ARG_STRING:
                *accessor->string(interp, arg_info, param_index) =
                    VTABLE_shift_string(interp, call_object);
                break;
              case PARROT_ARG_INTVAL:
                *accessor->intval(interp, arg_info, param_index) =
                    VTABLE_shift_integer(interp, call_object);
                break;
              case PARROT_ARG_FLOATVAL:
                *accessor->numval(interp, arg_info, param_index) =
                    VTABLE_shift_float(interp, call_object);
                break;
              default:
                if (named_used_list != NULL)
                    Parrot_hash_destroy(interp, named_used_list);
    ...
    if (err_check && VTABLE_elements(interp, call_object)) {
        /* We have extra positional args left over. */
        if (named_used_list != NULL)
            Parrot_hash_destroy(interp, named_used_list);

        /* FIXME Count required numbers */
        Parrot_ex_throw_from_c_args(interp, NULL,
            EXCEPTION_INVALID_OPERATION,
    ...
    /* Gather all named parameters into single hash */
    /* TODO It's slightly suboptimal and can be improved */
    named_params = collect_named_params(interp, call_object);
    ...
    while (param_index < param_count) {
        ...
        if (param_flags & PARROT_ARG_SLURPY_ARRAY) {
            ...
            if (named_params && named_params->entries) {
                /* Named argument iteration. */
                parrot_hash_iterate(named_params,
        ...
        if (!STRING_IS_NULL(param_name)) {
            ...
            if (named_params && Parrot_hash_exists(interp, named_params, param_name)) {
                PMC *value; /* FIXME Autoboxing!!! */
                ...
                value = (PMC *)Parrot_hash_get(interp, named_params, param_name);
                switch (PARROT_ARG_TYPE_MASK_MASK(param_flags)) {
                  case PARROT_ARG_INTVAL:
                    *accessor->intval(interp, arg_info, param_index) =
            ...
            switch(p->type) {
                case PARROT_ARG_INTVAL:
                  Parrot_hash_put(interp, hash, p->name,
                          Parrot_pmc_box_integer(interp, p->v.i));
                  break;
                case PARROT_ARG_FLOATVAL:
                  Parrot_hash_put(interp, hash, p->name,
                          Parrot_pmc_box_number(interp, p->v.n));
                  break;
                case PARROT_ARG_STRING:
                  Parrot_hash_put(interp, hash, p->name,
                          Parrot_pmc_box_string(interp, p->v.s));
                  break;
                case PARROT_ARG_PMC:
                  Parrot_hash_put(interp, hash, p->name, p->v.p);
                  break;
            }
        }
    }
...
Get pointer to next parameter to fill.

=item C<static STRING* string_constant_from_varargs(PARROT_INTERP, void *data,
INTVAL index)>

Throw an exception because named parameters aren't supported in C calls.
