  ...
  */
  ...
  while (info) {
    ...
    if (info->in && !info->in->shut_on_empty) {
      _ckvmssts(sys$qio(EFN$C_ENF,info->in->chan_in,IO$_WRITEOF,0,0,0,
			0, 0, 0, 0, 0, 0));
      info->waiting = 1;
      did_stuff = 1;
    }
...
popen_completion_ast(pInfo info)
{
  ...
  */
  ...
  if (info->out && !info->out_done) {             /* were we also piping output? */
    ...
    iss = sys$qio(EFN$C_ENF,info->out->chan_in,IO$_WRITEOF|IO$M_NORSWAIT,0,0,0,0,0,0,0,0,0);
  ...
  if (info->err && !info->err_done) {        /* we were piping stderr */
    ...
    iss = sys$qio(EFN$C_ENF,info->err->chan_in,IO$_WRITEOF|IO$M_NORSWAIT,0,0,0,0,0,0,0,0,0);
    if (iss == SS$_MBFULL) iss = SS$_NORMAL;
    _ckvmssts(iss);
  }
...
static unsigned long int setup_cmddsc(const char *cmd, int check_img, int *suggest_quote, struct dsc$descriptor_s **pvmscmd);
...
pipe_tochild_setup(char *rmbx, char *wmbx)
{
	...
	struct dsc$descriptor_s d_mbx1 = {sizeof mbx1, DSC$K_DTYPE_T, DSC$K_CLASS_S, mbx1},
                                d_mbx2 = {sizeof mbx2, DSC$K_DTYPE_T, DSC$K_CLASS_S, mbx2};
    ...
    if (p->chan_out) {
      if (myeof || kideof) {      /* pass EOF to parent */
        _ckvmssts(sys$qio(EFN$C_ENF,p->chan_out,IO$_WRITEOF, &p->iosb,
			  pipe_infromchild_ast, (ASTparm_t)p, 0,0,0,0,0,0));
        return;
  ...
  if (!err && !eof) {             /* good data to send to file */
    ...
    if (iss2 < 0) {
      ...
      if (p->retry < MAX_RETRY) {
        _ckvmssts(sys$setimr(EFN$C_ENF,QUAD_PTR(delaytime),pipe_mbxtofd_ast,(ASTparm_t)p,0));
        return;
      }
    }
    p->retry = 0;
...
vmspipe_tempfile(void)
{
  ...
  */
  ...
  fprintf(fp,"$ pif parrot_popen_in  .nes. \"\" then parrot_define/user/name_attributes=confine sys$input  'parrot_popen_in'\n");
  ...
  */
  ...
  if (!(sts & 1)) { 
    ...
    if (*in_mode != 'n' && ckWARN(WARN_PIPE)) {
      Perl_warner(packWARN(WARN_PIPE),"Can't pipe \"%*s\": %s", strlen(cmd), cmd, Strerror(errno));
    }
#endif
    *psts = sts;
    return NULL; 
  ...
  if (*mode == 'r') {             /* piping from subroutine */
    ...
    if (!info->fp && info->in) {
      info->done = TRUE;
      _ckvmssts(sys$qiow(EFN$C_ENF, info->in->chan_in, IO$_WRITEOF, 0,
			 0, 0, 0, 0, 0, 0, 0, 0));
...
my_pclose_pinfo(pInfo info)
{
  ...
  if (info->in && !info->in_done && !info->in->shut_on_empty)  /* EOF if hasn't had one yet */
    _ckvmssts(sys$qio(EFN$C_ENF,info->in->chan_in,IO$_WRITEOF,0,0,0,0,0,0,0,0,0));
  _ckvmssts(sys$setast(1));
  if (info->fp)
    fclose(info->fp);
  /*
        ...
        if (isdcl) {
          ...
          if (shebang_len > 0) {
            ...
            if (tmpspec[0] != '\0') {
              ...
              /*---------------------------------------------------*/
              iname = int_rmsexpand(tmpspec, image_name, ".exe", RMSEXPAND_M_VMS, NULL, NULL);
              if (iname != NULL) {
              if (cando_by_name_int(S_IXUSR,0,image_name,RMSEXPAND_M_VMS_IN)) {
                ...
                /*----------------------------*/
                iname = int_rmsexpand(tmpspec, image_name, ".", RMSEXPAND_M_VMS, NULL, NULL);
                if (iname != NULL) {
                  if (cando_by_name_int(S_IXUSR,0,image_name, RMSEXPAND_M_VMS_IN)) {
                    /* MCR prefix needed */
                    isdcl = 0;
