...
sub_param_type_def:
     type IDENTIFIER paramtype_list    { if ($3 & VT_NON_VOLATILE)
                                             $$ = mk_ident_nv(interp, $2, $1);
                                         else
                                             $$ = mk_ident(interp, $2, $1);
                                         $$->type |= $3; }
   | type STRINGC ADV_ARROW IDENTIFIER paramtype_list { 
                                         if ($5 & VT_NON_VOLATILE)
                                             $$ = mk_ident_nv(interp, $4, $1);
                                         else
                                             $$ = mk_ident(interp, $4, $1);
                                         $$->type |= $5;
