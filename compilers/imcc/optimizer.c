...
PARROT_WARN_UNUSED_RESULT
static int branch_reorg(PARROT_INTERP, NOTNULL(IMC_Unit *unit))
        __attribute__nonnull__(1)
        __attribute__nonnull__(2);
...
PARROT_WARN_UNUSED_RESULT
static int unused_label(PARROT_INTERP, NOTNULL(IMC_Unit *unit))
        __attribute__nonnull__(1)
        __attribute__nonnull__(2);
    ...
    for (ins = unit->instructions; ins; ins = ins->next) {
        ...
        if (((ins->opnum == PARROT_OP_add_i_ic ||
          ...
          || ((ins->opnum == PARROT_OP_add_n_nc ||
            ...
            if (ins->opnum == PARROT_OP_add_i_ic ||
                ins->opnum == PARROT_OP_add_n_nc)
                tmp = INS(interp, unit, "inc", "", ins->r, 1, 0, 0);
            else
                tmp = INS(interp, unit, "dec", "", ins->r, 1, 0, 0);
            subst_ins(unit, ins, tmp, 1);
            IMCC_debug(interp, DEBUG_OPT1, "%I\n", tmp);
            ins = tmp;
            changes = 1;
        ...
        if (((ins->opnum == PARROT_OP_add_i_i_ic ||
          ...
          || (ins->opnum == PARROT_OP_mul_n_nc_n &&
            ...
            tmp = INS(interp, unit, "set", "", ins->r, 2, 0, 0);
            IMCC_debug(interp, DEBUG_OPT1, "%I\n", tmp);
            subst_ins(unit, ins, tmp, 1);
            ins = tmp;
            changes = 1;
...
branch_branch(PARROT_INTERP, NOTNULL(IMC_Unit * unit))
{
    ...
    for (ins = unit->instructions; ins; ins = ins->next) {
          if (get_branch_regno(ins) >= 0) {
              r = get_sym(interp, get_branch_reg(ins)->name);

            if (r && (r->type & VTADDRESS) && r->first_ins) {
                next = r->first_ins->next;
/*                if (!next ||
                      STREQ(next->r[0]->name, get_branch_reg(ins)->name))
                    break;*/
                if (next &&
                      (next->type & IF_goto) &&
                      STREQ(next->op, "branch") &&
                      !STREQ(next->r[0]->name, get_branch_reg(ins)->name)) {
...
branch_reorg(PARROT_INTERP, NOTNULL(IMC_Unit *unit))
{
    ...
    for (i = 0; i < unit->n_basic_blocks; i++) {
        ...
        if ((ins->type & IF_goto) && STREQ(ins->op, "branch")) {
            r = get_sym(interp, ins->r[0]->name);
            if (r && (r->type & VTADDRESS) && r->first_ins) {
                Edge *edge;
        ...
        if ((ins->type & IF_goto) && STREQ(ins->op, "branch")) {
            ...
            if (end && (end->type & ITLABEL) &&
                ...
                for (cond = start->next; cond; cond = cond->next) {
                    ...
                    if (cond->type & IF_goto && STREQ(cond->op, "branch")) {
                        break;
        ...
        NOTNULL(SymReg *r), int what)
{
    ...
    for (bb = 0; bb < unit->n_basic_blocks; bb++) {
        ...
        for (ins = lr->first_ins; ins; ins = ins->next) {
            ...
            if (STREQ(ins->op, "find_global"))
                return reason=4, 0;
            if (STREQ(ins->op, "store_global"))
                return reason=4, 0;
            if (STREQ(ins->op, "push"))
                return reason=4, 0;
            if (STREQ(ins->op, "pop"))
                return reason=4, 0;
            if (STREQ(ins->op, "clone"))
                return reason=4, 0;
            /* indexed set/get ??? RT#46289, as index is ok */
            if (0 && STREQ(ins->op, "set") && nregs != 2)
                return reason=5, 0;
            /*
             * set P, P  - dangerous?
             */
...
is_invariant(PARROT_INTERP, NOTNULL(IMC_Unit * unit), NOTNULL(Instruction *ins))
{
    ...
    if (ok)
        return is_ins_save(interp, unit, ins, ins->r[0], what);
    return 0;
}
