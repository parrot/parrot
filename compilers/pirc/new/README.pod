...
When expanding macros, parameters are not handled. Also, braced arguments
are not handled right now.

=item * .NL() macro.

IMCC's implementation macros stores a newline for empty macros. This means
that you can write:

 .sub main
   print 42 .NL() print "foo"
 .end

This is not possible in pirc/new. Currently, I don't see a solution to fix this,
as consecutive newlines in the input file are collected together, and only a single
token is returned to the parser. This makes the parser much more cleaner.

=item * No (character) Escape!

IMCC can handle escape characters in double quoted strings. This isn't working
currently (but should be rather straightforward).
...
=item * Cygwin processable lexer spec.

The file C<pir.l> from which the lexer is generated is processable by Cygwin's default
version of Flex (currently 2.5.4). It must be noted, though, that the C<%option reentrant>
is not allowed with this version of Flex, so it's not clear whether the generated lexer is
suitable for parallel processing (re-entrant, thread-safe).
