...
Parrot_new_pmc_header_arena(struct Parrot_Interp *interpreter)
{
    ...
    new_arena->GC_data =
        mem_sys_allocate(sizeof(PMC *) * PMC_HEADERS_PER_ALLOC);
    memset(new_arena->GC_data, 0, sizeof(PMC *) * PMC_HEADERS_PER_ALLOC);
    new_arena->start_PMC =
        mem_sys_allocate(sizeof(PMC) * PMC_HEADERS_PER_ALLOC);
    memset(new_arena->start_PMC, 0, sizeof(PMC) * PMC_HEADERS_PER_ALLOC);
    new_arena->free = 0;
    new_arena->used = PMC_HEADERS_PER_ALLOC; /* Need to rethink the whole
                                              * arena free/used thing, as the
                                              * headers go into a single
                                              * allocation pool */
    new_arena->next = NULL;
    ...
    for (i = 0; i < PMC_HEADERS_PER_ALLOC; i++) {
        add_pmc_to_free(interpreter,
                        interpreter->arena_base->pmc_pool, cur_pmc++);
    }
}
...
new_pmc_header(struct Parrot_Interp *interpreter)
{
...

    {
        ...
        foo = interpreter->arena_base->pmc_pool->pool_buffer.bufstart;
        ...
        foo += interpreter->arena_base->pmc_pool->entries_in_pool;
...
alloc_more_buffer_headers(struct Parrot_Interp *interpreter)
{
    ...
    new_arena->start_Buffer =
        mem_sys_allocate(sizeof(Buffer) * BUFFER_HEADERS_PER_ALLOC);
    memset(new_arena->start_Buffer, 0,
           sizeof(Buffer) * BUFFER_HEADERS_PER_ALLOC);
    new_arena->free = 0;
...
new_buffer_header(struct Parrot_Interp *interpreter)
{
     ...
     * Thing. */
    {
         ...
         * pointer. Conveniently this is both the offset into a zero-based
         * array and the count for the next time */
        foo += interpreter->arena_base->buffer_header_pool->entries_in_pool;
        /* Dereference the buffer pointer to get the real string pointer */
        return_me = *foo;
        /* Count that we've allocated it */
...
static void
add_header_to_free(struct Parrot_Interp *interpreter, struct free_pool *pool, void
                   *to_add)
{
    Buffer **temp_ptr;
    /* First, check and see if there's enough space in the free pool. If
...
alloc_more_string_headers(struct Parrot_Interp *interpreter)
{
    ...
    new_arena->free = 0;
    new_arena->used = STRING_HEADERS_PER_ALLOC; /* Need to rethink the whole 
                                                 * arena free/used thing, as
                                                 * the headers go into a single
                                                 * allocation pool */
    new_arena->next = NULL;
...
new_string_header(struct Parrot_Interp *interpreter)
{
     ...
     * Thing. */
    {
         ...
         * pointer. Conveniently this is both the offset into a zero-based
         * array and the count for the next time */
        foo += interpreter->arena_base->string_header_pool->entries_in_pool;
        /* Dereference the buffer pointer to get the real string pointer */
        return_me = *foo;
        /* Count that we've allocated it */
        interpreter->active_Buffers++;
