...
PIO_stdio_init(theINTERP, ParrotIOLayer *layer)
{
    ...
    if (PIO_STDIN(interpreter))
        PIO_stdio_setbuf(interpreter, layer, PIO_STDIN(interpreter),
                         PIO_UNBOUND);
    return 0;
}
 ...
 */
INTVAL
PIO_stdio_setbuf(theINTERP, ParrotIOLayer *layer, ParrotIO *io, size_t bufsize)
{
    ParrotIOLayer *l = layer;
    ParrotIOBuf *b = &io->b;
...
INTVAL
PIO_stdio_setlinebuf(theINTERP, ParrotIOLayer *l, ParrotIO *io)
{
    /* Reuse setbuf call */
    int err;
    if ((err = PIO_stdio_setbuf(interpreter, l, io, PIO_LINEBUFSIZE)) >= 0) {
                ...
                const void *buffer, size_t len)
{
    ...
    if (len >= io->b.size) {
        ...
        PIO_stdio_flush(interpreter, layer, io);
        wrote = PIO_stdio_writethru(interpreter, layer, io, buffer, len);
        if (wrote == (long)len)
            return wrote;
        else {
            /* FIXME: Write error */
 ...
 */
size_t
PIO_stdio_writethru(theINTERP, ParrotIOLayer *layer,
                    ParrotIO *io, const void *buffer, size_t len)
{
    ParrotIOLayer *l;
    l = layer;
...
INTVAL
PIO_stdio_puts(theINTERP, ParrotIOLayer *layer, ParrotIO *io, const char *s)
{
    ParrotIOLayer *l = layer;
    while ((l = PIO_DOWNLAYER(l)) != NULL) {
        if (l->api->PutS) {
