...
What is a type signature? It is a string which represents the calling and
...
All type signatures follow the form of:

    Foo->Bar

where C<Foo> and C<Bar> are a list of zero or more Parrot datatypes. Here is
an example of a few type signatures and what they mean:
...
TODO: Multiple return values? Spaces? Is "->" valid ?

There are four main datatypes that can be used in Parrot Type Signatures:
...
type signature. It stands for "PMC invocant" and basically means SELF. C<Pi>
