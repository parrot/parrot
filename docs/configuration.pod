...
out if that command is connected to a specific compiler (like F<gcc>) would be an
automatic step.
...
F<lib/Parrot/Configure/Options/Conf.pm> and F<lib/Parrot/Configure/Options/ReConf.pm>:

=over 4

=item *

the list of valid command-line arguments in the array
C<@valid_options> and
...
James Keenan refactored the configuration system. 
