...
C<make fulltest> runs the test suite for all known working runcores and
optimizations levels and runs also several codingstd tests.
...
L<Parrot::Test> and the PIR module C<Test;More> help with writing tests.
...
are run by L<Test::Builder>-based modules such as L<Test::Simple> and L<Test::More>.

=item d

Those test files are run by L<Test::Harness>-based functionality such as
L<ExtUtils::MakeMaker>'s F<make test>, L<Module::Build>'s F<build test>, or
L<Test::Harness>'s F<prove>.

=item e

The extent to which the test files exercise all statements in the Perl modules
being tested is measured in coverage analysis using CPAN module L<Devel::Cover>.
...
B<TODO>. See L<Test::More> on how to do that.
