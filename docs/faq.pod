...
counts every time an object is references, or unreferenced. This
...
scheme. Since the code to manipiulate the reference counts I<must> be
...
that is ultimately just interrpeter bookkeeping, and not dedicated to
...
part, which lends itself well to processor's prefech mechanisms where
