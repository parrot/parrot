...
Added the "Parrot and Perl" section and "Why Reimplement Perl".  Incorporated Dan's Q&A items.
...
Gregor Purdy is working on a little language called Jako that targets Parrot bytecode directly.  (Available with the
Parrot distribution.)
...
Parrot has an odd license.  Most licenses--even open-source ones--aren't compatible with Parrot.  Anything licensed 
under Perl 5's terms is okay, and some things licensed under BSD or X licenses may be.  GPL licenses aren't okay.
...
It would be nice to write the Perl to Bytcode compiler in Perl, instead of 
...
decreases the amound of stack thrash we get.
