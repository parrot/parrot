...
B<Parrot_jit_normal_op> or B<Parrot_jit_cpcf_op> and may be implemented
...
until the B<pc> has the desired alignment. So the function can either
...
Jit code generated by the F<imcc> JIT optimizer used negative numbers
...
To obtain register offsets, another set of macros exists, that have C<OFFS>
in their names:
...
There are four mapped integer registers B<%edi>, B<%esi>, B<%ecx>, and
B<%edx>. The first 2 of these are callee saved, they preserve their
...
Also generated assembly code might be different.
