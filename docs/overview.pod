...
the first C<INTVAL> will specify the operation number, and the remaining
...
C<dec_i_c> takes two C<INTVAL>s as arguments, and decrements contents of the
integer register designated by the first C<INTVAL> by the value in the
second C<INTVAL>. Naturally, operations which act on C<FLOATVAL> registers will
use C<FLOATVAL>s for constants; however, since the first argument is almost
always a register B<number> rather than actual data, even operations on
string and PMC registers will take an C<INTVAL> as the first argument. 
...
tables mentioned above, you should be able to see how Parrot is
