...
C<malloc>ed memory.  This happens after L<"finalization">, and if resources
are adequate, may only happen as a side effect of program exit.
...
In the Parrot context a JAPH is a small script that prints the string
...
Acronym for Parrot Design Document. These documents describe the
features that the Parrot interpreter should implement.
...
POD is an acronym for Plain Old Documentation. This is for now the
prefered form for all kinds of documentation in Parrot.
