...
Parrot Intermediate Code. A medium-level assembly language for Parrot
that hides messy details like register allocation so language
compiler writers who target IMC don't have to roll their own.
...
run PBC files, PASM files or IMC files).
...
Parrot Intermediate Representation. The original name for IMC. Since
it has become the convention to name input files to IMCC with the
extension ".imc", IMC is a more natural name, although PIR is still
sighted regularly on the E<lt>perl-internals@perl.orgE<gt> email list.
