...
Acronym for Not Quite Perl (6).  NQP is designed to be a very small compiler for
...
Acronym for Parrot Compiler Toolkit, a complete set of tools and libraries
that are designed to create compilers targeting Parrot. The PCT consists of
a parser generator (PGE), a tree transformation tool (TGE), the Not Quite Perl
(NQP) language, the Parrot Abstract Syntax Tree classes (PAST), the Parrot
Opcode Syntax Tree (POST) and the HLLCompiler class. The latter can be used
as a compiler I<driver>, that invokes the  several compilation phases.
...
implemented by IMCC. There are 2 versions of PIRC: a handwritten, recursive-
...
PMC is an acronym for Parrot Magic Cookie. PMC classes are the primitives that
Parrot-based languages use to represent their fundamental types, such as Perl's
scalar values.
