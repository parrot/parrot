...
B<int>, B<float>, B<string> and B<pmc>.
...
Deprecated [RT#45453]. Use .HLL_map "CoreType", "UserType" instead.
...
Between B<.begin_return> and B<.end_return>, specify one or
...
Between B<.begin_call> and B<.call>, specify an argument to be
passed.  Available flags:
C<:flat>.

=item .result <var> [:<flag> ...]

Between B<.call> and B<.end_call>, specify where one or more return
...
PCC function call with B<.begin_call> and B<.end_call> directives.
I<var> can denote a global subroutine, a local B<identifier> or a B<reg>.
