...
(C<:main>) is an pragma that tells parrot that
...
more more virtual names onto the same physical register when it can
...
C<disassemble> on the C<*.pbc> output.
...
To accept values from such a function, use a parenthesized results list:
