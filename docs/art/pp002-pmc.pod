...
http://www.parrotcode.org/. Instructions for compiling
...
useful behavior. Here's a program that lists all of the PMC
types that are builtin to Parrot:

=head3 Example 1: list all PMC types
...
In this program, the C<typeof> opcode is used to determine
the type of each PMC available to Parrot. When given an
integer argument, C<typeof> will return the name of the PMC
type corresponding to that integer as a string.
