...
=item * Functions marked as C<static> can only be identified by address as
they have no "symbol name" for dynamic linking in the executable's symbol table.
Static functions will appears as C<(unknown)>.

=item * There must be some means available for walking the stack at runtime.  On
x86(-64)? the "stack pointer" must be in C<[re]sp> register.  For example, this
C<gcc> compliiation flag would break backtracing (except for functions that do
dynamic allocation on the stack as this optimization can no be allied to them).
C<perl Configure.pl --ccflags=-rdynamic>
...
it's available in the object code (for example, if parrot was built with C<-g>).

Since the C<Parrot_confess> symbol is I<always> compiled into parrot it can be used as a break point to obtain a backtrace.  Here is an example of doing this with gdb and a version of parrot compiled with <gcc> and the C<-g> flag.
