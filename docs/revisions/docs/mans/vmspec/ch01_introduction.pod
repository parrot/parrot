...
docs/mans/vmspec/ch01_introduction.pod - Introduction. 
...
plans to merge Python and Perl into a new language called Parrot. I<See>
http://www.perl.com/pub/a/2001/04/01/parrot.htm.

=head2 1.2 Overview

=item The Parrot Interpreter
...
=item The Software CPU

Like all interpreter systems of its kind, the Parrot interpreter is a virtual 
machine; this is another way of saying it is a software CPU. However, unlike 
other VMs, the Parrot interpreter is designed to more closely resemble hardware 
CPUs.

For instance, the Parrot VM employs a register-based architecture, rather than 
a stack-based architecture. It also has very low-level operations, more similar 
to Java's than to the medium-level ops of Perl, Python, and the like.

The reasoning behind this decision is, by resembling the underlying hardware 
to some extent, it's possible to compile down Parrot bytecode to efficient 
