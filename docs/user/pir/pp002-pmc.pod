...
L<http://www.parrotcode.org/>. Instructions for compiling
Parrot are available in the Parrot distribution itself. All
code examples in this article were tested with Parrot 0.4.5
...
Utilizing a slightly higher-level syntax called PIR (Parrot
...
boolean false and the conditional on line will cause the
program to end.
...
(whether it be a register or local variable), Parrot puts
the number of elements in the aggregate into the integer
thing. (e.g., if you had a PMC that held 5 things in C<$P0>,
the statement C<$I0 = $P0> assigns 5 to the register C<$I0>)
...
square brackets just like you would in Perl and other
...
corresponds to one opcode. So, while there's an opcode to
index into an aggregate and an opcode to print a string,
there is no opcode to do I<both> of those things.
...
PMC type. If you assign the result to an integer thing, you
get the PMC type number.
