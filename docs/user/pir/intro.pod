...
parrot virtual machine. It corresponds closely to parrot bytecode,
and, while it provides direct access to the opcodes, it also
abstracts common operations and conventions into a
syntax that more closely resembles a high-level language.
...
In order to test the PIR code in this article, a parrot virtual
...
Each register name begins with the prefix above and is followed by a number.
For instance, the integer registers are
...
If you examine the bytecode generated from compiling PIR, you may notice
that the register number in the bytecode may not correspond directly to
the register number you specified in PIR - don't worry about it - You can
use as many registers as you want, and the register allocator will
allocate them as needed, and handle mapping the $ registers to the actual
slots in the VM.
...
conventions, let's compare some different PIR syntax.

If I want to add two integer registers using opcodes directly I would use the
...
PIR includes infix operators for these common opcodes. You could write
this same code as
...
Another simplification is branches. Basically, when I want to
test a condition and jump to another place in the code, I could write
the following code:
...
learned, let me show you how to do read input from the user.  Just
