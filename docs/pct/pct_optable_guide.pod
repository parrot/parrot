...
In order to preven very big parse trees (which is rather inefficient),
it's much better to do bottom-up parsing for expressions such as these.


=head1 HOW DOES AN OPTABLE WORK?

{{explain how this works; is this needed? }}
...
According to standard mathematical rules, C<+> has lowest precedence,
then comes the multiply operator, and the power operator has the highest
precedence. Still, there are two ways to parse this (assuming C<+> and
C<*> are left associated, which they are):
...
which yields a complete different result. The latter option is the correct
one, according to what you learnt in high school. The power operator takes
its right operand and applies that to its left operand.
...
In other words, does the C<-> operator first evaluate its right operand or
first its left operand. According to standard rules, the C<-> operator
is C<left> associated, which means the first option is correct.

This associecity is expressed using the C<assoc> trait, like so:

 proto 'infix:-' is assoc('left') ...

If you don't specify the association, it defaults to C<'left'>. Other
possible values are: C<'right'> and C<'none'>.

{{ explain what 'none' implies }}

{{ check whether I explained this correctly. This is how I understood
   it. kinda. }}
