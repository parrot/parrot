...
=item * Can only replace inside string or index after end of string
...
(« and »). The best solution to solve this is to comment out the whole table
except for a single operator, and try to find the error using a binary search.
