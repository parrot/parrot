...
storage for data items has to be allocated. This is done
automatically, when the initializer is attached to the Struct PMC.
...
Given a C function that returns a structure containing a callback
...
Each contained structure needs its own UnManagedStruct initializer.
The UnManagedStruct of the contained structures has to be attached to
the structure type PMC as the property B<"_struct">.
...
Not all datatypes are implemented. Alignment is barely tested on
different machines. Arrays of structures aren't handled yet. Passing
nested structures to C isn't finished.
