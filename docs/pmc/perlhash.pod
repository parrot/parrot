...
When using a key that does not exist in the hash, the value returned
...
command will return true if the element in that position is a
...
You can remove (or delete) an element from the hash using the
C<delete> command:
...
where C<P0> is the Perl Hash PMC and C<I0> the register where to store
the hash size. You can use an Perl Hash PMC on an C<if> or C<unless>
command, where it will be checked for the number of elements.
