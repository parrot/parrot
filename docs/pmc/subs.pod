...
Parrot Calling Conventions) are stored in the constant table
and can be fetched with the B<find_global> opcode.
...
=head3 Program entry point

Exactly one subroutine in the first executed source or byte code file
may be flagged as the "main" subroutine, where executions starts.
...
=head3 Automatically loaded initializer code
...
When a subroutine label is prefixed by B<.pcc_sub>, the name of the
subroutine (i.e. the label) gets stored in the global stash.
