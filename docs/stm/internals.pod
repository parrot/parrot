...
The STM implementation is based on Ennals' paper, see
L<http://www.cambridge.intel-research.net/~rennals/052_Rob_Ennals.pdf>.
...
The notion of read-onlyness is obtained first from C<vtable.tbl>'s
...
C<mmd_ensure_writable> in C<mmd.c>.) For other methods, C<find_method>
is overriden in the read-only variant to check the "write" property
on the method that would be returned. (See also C<Parrot_mark_method_writes>
in C<inter_misc.c>.) If it is set, then C<PMCNULL> is returned, 
making the method lookup fail. 
...
The last read and write record in use.
...
Note that because version numbers in this saved log are out-of-date, 
replaying it is not likely to be useful.
...
sake of systems with few processors, it also calls C<YIELD> after 10 loops
to give other programs a change to run. While waiting it does
...
that record is used. Otherwise C<wait_for_version()> is called to get
...
If the C<PARROT_ATOMIC_PTR_CAS> fails and C<wait_for_version> was used,
C<find_write_record()> keeps trying unless the current transaction or
some outer transaction is aborted. Otherwise, if C<PARROT_ATOMIC_PTR_CAS>
fails, the current transaction is marked as aborted.
...
that the transaction was aborted by some other thread. It is at this point
that the transaction logically occurs if it commits at all. (The
