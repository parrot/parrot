...
There is one built-in compiler objects, for PIR.
The PIR compiler objects is a Sub PMC that takes a single string
argument and returns an array PMC containing a list of all the compiled
