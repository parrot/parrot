...
PIR is a very low-level language by any programming
standards, even though it supports some important features of
high-level dynamic languages, and has some symbolic syntax features.
...
tasks easier. Libraries written in PIR can be easily included
...
wrapper libraries are provided to create an interface that programs
