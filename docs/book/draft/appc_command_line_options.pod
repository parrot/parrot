...
=item -d,--imcc-debug [R<hexbits>]

Turn on debugging output for the compiler. The C<-d> switch takes an optional
argument, which is a hex value of debug bits. The individual bits are shown in
...
=cell Rearrange PASM registers with the most-used first. (currently ineffective)
