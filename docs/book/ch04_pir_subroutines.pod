...
You can get it's name or I<change> it's name:
...
If the subroutine is lexical, you can get it's C<:outer> with the C<get_outer>
...
namespaces. Each HLL compiler gets it's own HLL namespace where it can store
it's compiled data. Each HLL namespace may have a large hierarchy of other
