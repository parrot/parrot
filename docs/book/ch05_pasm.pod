...
This example returns the integer representation of C<String>,
which is 36.
...
Here, the name "String" returns 36, and the name "INTVAL" returns
-100.
...
inherits from C<Int>X<Int PMC>, which is derived from the
...
C<Int>, and assigning C<undef> morphs it to C<Undef>:
...
suffix, as in C<substr_r>. Please note that C<substr_r> has been
deprecated.
