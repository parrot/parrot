...
virtual CPU.  PASM is an interesting language with a mix of mix of
features. As an assembly language it is decidedly low-level with many such
aspects: flow control based on branches and jumps; and direct manipulation
of values in the software registers and on the stacks.  Basic register
operations or branches generally translate into a single CPU instruction.
N<This means the JIT run time has a performance of up to one PASM instruction
per processor cycle.> On the other hand, because it's designed to
implement dynamic high-level languages, it has support for many advanced
features such as lexical and global variables, objects, garbage collection,
...
labels are often all caps to make them stand out from the rest of the
sourace code more clearly.  A label definition is simply the name of the
label followed by a colon. It can be on its own line:
...
Unicode.N<This conversion isn't fully implemented yet. We're working on it.>
...
Once the F<parrot> executable has been built, You can generate a complete and
current list of valid PMC types by running this command within the main Parrot
source directory:

  $ perl tools/dev/pmctree.pl src/pmc/*.pmc
...
String operations work with string registers and with PMCs that
...
suffix, as in C<substr_r>.N<Please note that C<substr_r> has been
deprecated.>
...
X<Parrot;stacks;>
Parrot has a control stack to handle the passage of control between
...
calls is listed in A<CHP-5-TABLE-4>Table 5-4.
