...
are not attatched to a numeric value, but instead are enforced by
...
collection in a simillar manner to strings.
...
Comparision returns a big number which is equal to 1, 0 or -1 if the
...
passed to C<BN_nonfatal()>, this examines the error which has occured
and the current context to determine which class of error has occured.
