...
If a PMC has this set, it is responsible for marking all buffers and
...
=item void bitwise_shl_keyed(INTERP, PMC* self, KEY* key, PMC* value, KEY* value_key, PMC* dest, KEY* dest_key)
...
=item void bitwise_shr_keyed(INTERP, PMC* self, KEY* key, PMC* value, KEY* value_key, PMC* dest, KEY* dest_key)
...
XXX: this is out of date w.r.t. the implementation -- what does this method
actually do?
...
the interpreter should just continue to the next instruction. (If the
