...
The following vtable functions have a variation for INTVAL and PMC
values.  They have keyed and non-keyed versions.
...
=item void bitwise_or(INTERP, PMC, value, dest)

=item void bitwise_and(INTERP, PMC, value, dest)

=item void bitwise_xor(INTERP, PMC, value, dest)
...
=item void bitwise_not(INTERP, PMC, dest)

Applies a bitwise negation to PMC and stores the result in dest.
...
=item void logical_not(INTERP, PMC, dest)

Does a logical not on PMC and stores the result in dest.
