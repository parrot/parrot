...
Fetch the entry that's at position Y from the top of the stack. This
does not remove an entry from the stack, merely fetches the entry off
it.

0 is the entry from the top of the stack, 1 is the entry immediately
previous to that, and so on. Entry -1 is the very bottom-most entry in
the stack. (While the stack may be a tree when looked at from the bottom up, you
don't have access to any other branches when looking this way)
...
Find a routine named Z, in library Y (which you did, of course, open
with loadlib), and put a sub PMC onto X for it. You can call this sub
...
pointers. Presumably prevous calls would have set those pointers up
properly.
