...
and the query METHOD "get_lexinfo()" (to return it).
...
Closure a new LexPad attribute.  The LexPad is then populated with pointers to
the current I<enclosing> LexPads.  The definition of "enclosing" is not
obvious, however.

The algorithm used to find them is a loop of the following steps, starting
with $sub set to the running Subroutine (which is a Closure):
  ...
  3. Set $sub to $sub.outer.  (That is, the textually enclosing subroutine.)
     But if $sub has no outer sub, END LOOP.

NOTE: The C<newclosure> opcode should check to make sure that the target
Subroutine has a :outer() attribute that points back to the currently running
Subroutine.  This is a requirement for closures.
