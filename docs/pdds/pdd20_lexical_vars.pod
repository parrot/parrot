...
associated LexInfo) and the query METHOD "get_lexinfo()" (to return
it).
...
create a LexInfo for it at compile time, and therefore cannot create a
LexPad for it at run time.
...
with pointers to the current I<enclosing> LexPads.  The definition of
"enclosing" is not obvious, however.
...
development -- if ever -- that call frames will be available to user
code as PMCs.  Until then, the interpreter and continuation PMCs will
be the interface to use to get frame info.
...
It's likely that this interface will continue to be available even
once call frames become visible as PMCs.
