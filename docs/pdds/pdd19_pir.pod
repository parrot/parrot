...
Execute immediately after being compiled.

As an example, the file C<foo.pir> contains:
...
Executing C<foo.pir> will run both C<foo> and C<bar>. If C<foo.pir> is compiled
to bytecode both subs will be executed during compilation. If the resulting
bytecode is loaded by another program, neither Sub will rub.

Note that C<:immediate> and C<:postcomp> routines execute together in IMCC, and
may execute in any order. Other compiler frontends may have different semantics
for these flags or may remove them all together.
