...
=item (a) the PMC is either an aggregate or a scalar containing a reference to
an aggregate, and

=item  (b) the target register has the SLURPY bit set,
...
=item (a) if the value is an array, pass its elements in the normal way (FLAT
style);

=item (b) if the value is a hash, pass its elements as a list of key/value
...
=item 9     NAMED (FLAT or string constant only)
...
=item 9     NAMED (SLURPY or string constant only)
