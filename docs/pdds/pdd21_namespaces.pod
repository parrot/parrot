...
elements of the array in $P3.  If no parameter is passed to the method,
or if $P3 is a null PMC or an empty array, C<get_namespace> retrieves
the base namespace for the HLL.  It returns a namespace PMC on success
and a null PMC on failure.
