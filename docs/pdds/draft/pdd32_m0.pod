...
C<*$1> is used as the target address and C<*$2> is the name of the target
chunk.
...
little-endian byte order.  INSP values may be either 4 or 8 bytes, depending on
what the underlying platform supports.  N registers always be 8 bytes long.
Converting between M0 bytecode files of differing type sizes will be supported
implicitly through the disassembly and reassembly of .m0b files.
...
greater precision than the target implementation is capable of supporting.  In
this case, the assembler should fail with an appropriate error message.
