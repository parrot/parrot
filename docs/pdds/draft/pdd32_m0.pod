...
NOTE: This suggestion omits the mechanism for creating and destroying the space
where an ffi thunk is stored.  We need to figure out what this would look like.

NOTE: define "thunk" as it's used in Parrotland.  Different fields and
languages use it differently.
...
M0 will know enough about FFI to create a reusable thunk that takes M0 values,
...
the C function through the generated thunk.

=item * set_arg thunk_handle, arg_type, arg_src

set_arg takes three arguments: a thunk handle (or possibly a pointer to a
region of memory), an argument type and an argument source.  The thunk handle
indicate which thunk is being operated on; it can be any value that uniquely
identifies a thunk.  arg_type is a constant indicating the type of the
argument.  The values of this argument will be similar to those used by libffi
(https://github.com/atgreen/libffi/blob/master/doc/libffi.info ).  arg_src
indicates which M0 INSP register should be used to populate or recieve the
value of the approrpiate argument.
set_arg will need to keep track of the last arg to be set internally.

=item * set_ret thunk_handle, arg_type, arg_dest

set_ret is similar to set_arg, except that it always sets the type and
destination of the return value from the thunk.

=item * ccall thunk_handle

ccall uses an existing thunk to call into a C function.

example code for calling no arg, no ret function:
.local int lib, thunk
lib = dlopen 'libhello.so'
thunk = dlfunc lib, "hello_func"
ccall thunk
...
=head3 Other Possible Implementations:

An alternative approach is to specify the entire signature at once.  This has
the advantage of making allocation simpler (since the length is known once the
op is parsed) and of making life simple for M0 implementations which compile
directly to C and don't need an explicit ffi, but it also complicates to the
parser and necessitates additional complexity in the bytecode format to support
signature storage.

#always use explicit signature, avoids assumption that thunks are the underlying mechanism
gen_call $P0, [INT32, INT32, $I0, INT32, $I1]
callc 'multiply_int32', $P0"

#more objecty version of above
$P0 = <funcptr>;
callc $P0, [INT32, INT32, $I0, INT32, $I1]
