...
Examples:
  ...
  # point register 3 at variable 257 (4 * 256 + 2 * 1)
  set_var 3, 4, 2
...
variables segment, a metadata segment and a chunk name.
  ...
  opcode_t : number of opcode_t-sized units in this segment
  opcode_t : offset of variables segment relative to the start of the file
  opcode_t : offset of metadata segment relative to the start of the file
  opcode_t : offset of bytecode segment relative to the start of the file
  opcode_t : size of chunk name
  [
    opcode_t : chunk name
  ]
