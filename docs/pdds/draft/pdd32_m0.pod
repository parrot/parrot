...
256 ops are nothing more than an array of 256 uint32-sized chunks of data.
...
the file.  It will have the following structure:
  ...
  uint32 : M0_DIR_SEG
  uint32 : number of chunks in this segment
  uint32 : number of bytes in this segment (including this header)
  [
    uint32 : offset of a constants segment
    uint32 : size of chunk name in bytes
    [
      char : chunk name
    ]
  ]
...
Data in constants table must be explictly loaded into registers as needed.
  ...
  uint32 : M0_CONST_SEG
  uint32 : number of constants in this segment
  uint32 : number of bytes in this segment (including this header)
  [
    uint32 : number of bytes in constant data
    ...    : constant data
  ]
...
provides a way to map values to names and bytecode offsets.
  ...
  uint32 : M0_META_SEG
  uint32 : number of entries in this segment
  uint32 : number of bytes in this segment (including this header)
  [
    uint32 : bytecode offset relative to the start of the current bc seg where this data becomes effective
    uint32 : offset into constants table for the name of this piece of metadata
    uint32 : offset into constants table for the value of this piece of metadata
  ]
...
bytecode.
  ...
  uint32 : M0_BC_SEG
  uint32 : number of ops in this segment
  uint32 : number of bytes in this segment (including this header)
  [
    char : opcode
    char : arg1
