...
Free the region of memory at C<*$1>.  C<$2> and C<$3> are ignored.  This is a
direct interface to the underlying implementation's C<free()> implementation
(or equivalent) and does not go through ffi.
...
=item * load_cref - point a register at a constants table slot

Point C<*$1> at a slot in the cosntants table. C<$2> contains the high byte of
the index, C<$3> contains the low byte.  Both C<$2> and C<$2> are treated as
1-byte immediate values.  Writing to the constants table is invalid.  Code that
needs a modifiable version of a value from the constants table must first copy
it into an allocated region of memory.

The constants table is serves as a location where constant data stored in
bytecode can be accessed.

Examples:

  # point register 0 at constant 123 (0 * 256 + 123 * 1)
  load_cref 0, 0, 123

  # point register 3 at constant 1026 (4 * 256 + 2 * 1)
  load_cref 3, 4, 2

=item * load_cval - copy a value from the constants table into a register

Copy a value from the constants table directly into a register.  This will copy
a register-sized number of bytes into C<*$1>, using C<$2> and C<$3> as the
index into the constants table, similar to C<load_cref>.  This is used to copy
constant-sized values such as integer and numeric values into a register.
Using C<load_cval> on a non-numeric constant is not valid and may be checked in
the future.
...
type:
    ...
    4       PCF - parent call frame
    5       CHUNK - the name of the currently-executing chunk
    6       CONST - pointer to constants table
...
TODO: Are we storing crypto hashes in M0 source and bytecode, or only bytecode?
