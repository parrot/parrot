...
=head3 Control Flow (2 ops - maybe)
...
The current proposal is that control flow be done by modifying the current
context's PC.   A basic example of how to do this is below.  Note that this
method of control flow may not result in efficient jitted code.  Explict
control flow may be a better idea.

    i = 0
    max = 10
  loop_start:
    #if i == max, goto loop_end
    delta = cmp max, i    #delta is 1 if max != i
    jump  = mul_i jump, 4 #jump  is 4 if max != i, 0 otherwise

    PC    = add_i PC + jump
    PC    = loop_end      #this instruction is skipped if max != i

    #do some kind of interesting work here
