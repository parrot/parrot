...
Unconditionally transfer control flow to the bytecode offset in C<*$1> within
the current bytecode segment specified in C<*$2>.  If C<*$3> is 0, C<*$1> is
treated as the number of a chunk.  If C<*$3> is non-zero, C<*$2> is treated as
...
Treat C<*$2> and C<*$3> as integer or floating-point values, subtract C<*$2>
...
Write C<*$3> bytes from C<*$2> to the memory location starting at C<*$1>.
...
Point C<*$1> at a slot on the variables table.  C<$2> contains the low byte of
the index, C<$3> contains the high byte.  Both C<$2> and C<$2> are treated as
