...
65536 slots.   Registers can be pointed at different slots with the C<load_var>
...
In descriptions, C<$0> refers to the value of the first argument to an op,
C<$1> to the second and C<$2> to the third.  Any op can operate on any
...
In this documentation, as in C, C<$0> refers to the value of a certain
register, where C<*$0> refers to what that register points at.
...
Explicitly do nothing.  C<$0>, C<$1> and C<$2> are ignored.
...
=item * store_mem - set a region of memory

Write C<*$2> bytes from C<*$1> to the memory location starting at C<*$0>.
Segfaults may result.  Be careful.

=item * load_mem - get a region from memory

Write C<*$2> bytes from the memory location starting at C<*$1> to C<*$0>.
Segfaults may result.  Be careful.

=item * load_var - point a register at a variables table slot

Point C<*$0> at a slot on the variables table.  C<$1> contains the low byte of
the index, C<$2> contains the high byte.  Both C<$1> and C<$2> are treated as
...
variables segment, a metadata segment and a chunk name.
