...
=item * new_fnh func_handle, arg_count

This allocates a new function handle which can deal with the specified number
of arguments.  Its internals are implementation-specific.

=item * free_fnh func_handle

This op destroys an existing function handle.

=item * dlopen lib, 'libfoo.so'

This op calls the libc dlopen function and returns an opaque handle for the
library.  This handle is used by C<dlfunc> to find a a function pointer.

=item * dlfunc func_handle, lib, "function_name"

This op looks up the function pointer for C<function_name> in the library for
which C<lib> is a handle, and puts the pointer into the function handle
C<func_handle>.
