...
control flow and C<goto_chunk> for moving between chunks.  While this is all
that's strictly necessary for Turing completeness, a bit more complexity is
needed to effectively implement constructs such as continuations, exceptions
(and exception handlers), coroutines and so forth.  The recommended structure
for non-trivial control flow is continuation-passing style, with M0's call
frame structure serving as the continuation.  Members elements of a call frame
may be manipulated using the C<set>, C<set_imm>, C<set_ref> and C<deref> ops.
