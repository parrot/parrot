...
garbage collection. At the beginning of the mark phase, the C<PObj_is_live_FLAG>
and C<PObj_is_fully_marked_FLAG> are both unset, which flags the PMC as presumed
dead (white). The initial mark phase of the collection cycle goes through each
PMC in the root set and sets the C<PObj_is_live_FLAG> bit in the C<flags> member
...
The fourth combination of the two flags, where C<PObj_is_live_FLAG> is unset and
...
C<PObj_is_live_FLAG> is set and the C<PObj_is_fully_marked_FLAG> is left unset),
