...
PBC should be cross-platform and in the future cross-version compatible.
...
Though we aim at cross-version portability, it is not implemented yet. So we
currently exit when reading an incompatible bytecode file version number.
...
These versions are listed in the PBC_COMPAT file, sorted with the latest
