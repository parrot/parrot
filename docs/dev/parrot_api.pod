...
The C<PARROT_API> macro exists for the purpose of annotating symbols that
...
Incorrect usage of C<PARROT_API> can break the build on some platforms,
especially Win32. The rules for how to use it are as follows.

=over 4

=item If you decorate a function with C<PARROT_API> in a .c file, you must
also decorate the symbol with C<PARROT_API> in all .h files that mention it.

=item If the symbol appears in more than one header file, all of them must
have C<PARROT_API> or none of them should. Some with and some without won't
work.
