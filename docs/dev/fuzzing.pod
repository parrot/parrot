...
harddisc space.
Use a fast kvm VM, only one core is needed.
...
I did not dare yet to fuzz C<nqp> or C<perl6> yet,
as it would be even slower.
...
Adding our regular pir and pasm keywords helps creating more meaningful permutations.
    ...
    perl -e'`echo $_ > afl-testcase2/keywords/$_` for qw(.sym .arg prototyped non_prototyped .class
    .endclass .param inc dec new defined global clone .call .result .return .local .const .globalconst
    ...
    .end_yield .loadlib .namespace .endnamespace .macro .include int float string pmc
    ne eq le lt ge gt == =head1 =end =pod)'

    afl-fuzz -m4000 -i afl-testcase2 -x afl-testcase2/keywords -o afl-out2 -- parrot -r @@

=head2 pbc packfiles

afl is more used to fuzz binary input to check the PackFile reader.
