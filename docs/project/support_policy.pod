...
releases:
  ...
  - Receive update releases for critical bug/security fixes (1.0.1, etc).
  - Have documented deprecations between each supported release.
  - Are intended for distribution packaging.
  - Are intended for users.

The developer releases happen every month between two supported
releases. The developer releases:

  - Track the latest monthly feature additions and removals from trunk.
  - Do not receive bug/security releases. (Fixes appear in the next
...
not include a platform in our offical support list (even with a
