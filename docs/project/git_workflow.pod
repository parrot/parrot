...
=head2 Syncing branches

=head2 Merging a Branch

When you're ready to merge your changes back into master, use the C<git merge>
command. First, make sure you are on the master branch with C<git checkout
master>. Now, to merge the branch user/foo into master:
