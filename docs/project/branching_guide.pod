...
To create a branch, use the C<svn copy> command.
  ...
  $ svn copy https://svn.perl.org/parrot/trunk \
         ...
         -m "Creating branch for <purpose of branch>."
...
number of your last synchronization (or the revision number of the initial
branch creation if this is the first synchronization), and the current highest
revision in trunk (you can get this by running C<svn update>).
...
(In the case of a completely disasterous merge, you can C<svn revert> your
...
Use a meaningful commit message, something like:
  ...
  [<yourbranchname>] Bringing the <yourbranchname> branch up-to-date
  with trunk r30371.
...
tests for their language or tool (or, didn't run *all* the tests for their
...
In the C<-r> option pass it the revision number where the branch was created,
and C<HEAD> to tell it to grab all changes committed in the branch from the
time you created the branch to the most recent change.
...
find it by clever dective work. But, its definitely easier to just keep it in a
text file.)
...
C<svn commit>. Use a meaningful commit message something like:
  ...
  [<yourbranchname>] Merging the <yourbranchname> branch into trunk for
  r30235 to r31667.

Delete your branch from the repository:

  svn delete https://svn.perl.org/parrot/branches/<yourbranchname> \
          -m "Removing <describe branch> from the repository"
