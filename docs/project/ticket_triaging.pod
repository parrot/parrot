...
architecture, and versions of included libraries.  The file F<parrot_config.pasm> may be useful.

=item Steps to reproduce.

At what point did you see the failure?  Can you reproduce it?  Do you have a small PIR program which demonstrates the problem?

=item Failure diagnostics.

Verbose diagnostics from C<prove -v> -- including all error messages and diagnostics -- are often necessary to resolve test failures.
...
Supposedly it's better customer relations to use I<issue(s)> instead of
I<problem(s)>.
