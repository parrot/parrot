...
Announce the new release to parrot-porters@perl.org and to the IRC channel #parrot
...
Make sure everything works:
 ...
 $ make -s all world html 2>&1 | tee make_all_world_html.log
 $ make    fulltest       2>&1 | tee make_fulltest.log

Note that running 'make fulltest' takes a while and that separate harnesses
are being run. It is not necessary to quiet all the codingstd tests for a release.
...
When all is well, then commit your changes:
 ...
 svn diff | more
 svn commit -m 'Changes for Parrot release ....'

Write down the revision number from this commit. You will need it later.
