...
Also update the URL in F<parrot.spec> for "Source0" to point to the
current release manager's CPAN directory. [Really, we should provide a
more standard location for the release tarball.]
...
properly part of the Parrot distribution (generally these are modules that have
been copied into the repository from CPAN). 
...
to PARROTRE and add all the other release managers as co-maintainers.
