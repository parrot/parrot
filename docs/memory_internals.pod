...
Garbage collector. A garbage collector is a system that finds memory
objects that are not being used by the system, and frees them
automatically. Our used garbage collection is broken into two distinct
phases: the I<mark phase> that searches for unreferenced objects, and
the I<sweep phase> that frees all dead objects.
...
provide this, such as the glibc malloc based on Doug Lea's F<ptmalloc>.
...
F<src/gc/api.c>, F<src/gc/gc_private.h>, F<pobj.h>, F<mark_sweep.[ch]>,
F<alloc_resources.[ch]>, F<string.[ch]>. Other garbage collector
