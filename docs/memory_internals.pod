...
allocated early in startup e.g. the names of the pools themselves.
...
Buffer_like objects of the same size are maintained in the
...
as above and does reuse free()ed items in a memory conserving way.
...
their existence, this is, when DOD detected, that the managing buffer
header is not used anymore, they are free()ed. That's all. The underlying
...
Both allocation schemes therefore use a I<Buffer_Tail>. After the end of the
