...
of similar items. These can be roughly divided into two kinds: pools
holding fixed sized items and pools holding variable sized items.
...
pools not alive at the end of the detection phase are considered dead.
...
C<struct Memory_Pool> are for variable-size objects, such as string buffers.
Pools of type C<struct Small_Object_Pool> are for fixed-size objects such
as headers or PMCs.
...
part after the buffer structure.
...
If the free list is empty then a DOD run is initiated. This may be able to
find some dead objects and free them up for immediate reuse. However, if
there are no objects that can be immediately recycled, a new arena will be
allocated.

These fixed-sized objects are never returned to the operating system
during the lifetime of an interpreter, they just get reused or recycled.
Eventually, this policy may change and unused arenas may be returned to
the OS.
