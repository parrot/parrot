...
tuning and debugging.  See L<About runcores> below for details.
...
this, partly because it is expected that no one runcore will perform
optimally on all architectures (or even for all problems on a given
architecture), and partly because some of the runcores are specialized
for debugging.
...
The switched core eschews a bunch of tiny op functions in favor of
cases in a large switch statement:
...
function calling.  On older systems, it may fail to compile
altogether.
