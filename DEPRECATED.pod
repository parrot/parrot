...
=item * C<PCCINVOKE> [post 0.5.2]

The syntax for passing named arguments to C<PCCINVOKE> will change from
:named['key'] to :named('key') to better match PDD 3. See RT #42292.
...
=item * C<DYNSELF> [post 0.5.2]

Replaced by C<SELF>. (Old behavior of C<SELF> changes to C<STATICSELF>.)
...
=item * unneeded type names

Since Parrot 0.4.16 all type names for .local other than string, num, int, and
pmc are deprecated. The option to use C<float> for C<num>
or a basic PMC name like C<Array> for C<pmc> will be removed soon.  [RT#42769].
