...
=item * C<get_*_keyed_int> VTABLE methods change the size of Resizable*Arrays
[post 0.7.1]

The C<get_{String|PMC|Integer|Number|Boolean}_keyed_int> VTABLE methods
increase the size of Resizable{String|PMC|Integer|Number|Boolean}Arrays when
they're used to search for an element past the end of the array.  This behavior
is deprecated and will be removed after the 0.7.1 release.
...
=head2 P6object class creation

=over 4

=item P6object .new_class('Foo::Bar') will create ['Foo';'Bar'] [post 0.7.1]

See RT #58932.  Strings passed as class names to P6object will be
automatically separated on double-colons.
