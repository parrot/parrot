...
=item Create module Lamguage::m4 or Text::m4

=back

=head1 Features

The feateres that need to be implemented are orded by chapters in the
...
=item Recognize builtin macros

=item Recognize macros with arguments

=item Rescanning evaluated macros
...
=item Stack for macro definitions
...
=item implement all of them

=item Use POSIX regexes

=item Implement the builtin function patsubst

=back

=head2 Chapter 11 Macros for doin arithmetic

=over

=item implent all of them

=item Arithmetic expression parser, bc ???
