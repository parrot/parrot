...
Code for a compilation unit is created as soon as the end of the unit
is reached.
...
to code in different units. So lexicals (e.g. for nested subs) have to
be managed by means of PASM statements. This might change in future if
needed.
