...
=item * B<prototyped>, B<pnon_rototyped>

Specifying these in the B<subroutine> is unimplemented.
This must currently be empty.
...
continuation, or the sub only. For the latter case an B<invokecc> gets
emitted.
...
The B<.param> function params must be the first statements in the
sub if any.
