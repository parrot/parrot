...
=item * userdata F<languages/lua/type/userdata.pir>

This type allows OO extension.

Currently, C<file> in I/O library is an C<userdata>.

=item * thread F<languages/lua/type/thread.pir>

This type is used to implement coroutines.
The current implementation use the library Parrot::Coroutine.

=item * table F<languages/lua/type/table.pir>

This type is I<the> data structure of the language.

=back

F<languages/lua/type/base_lua.pir> provides an abstract base class for 
Lua types (table, userdata, thread).

F<languages/lua/pmc/luabase.pmc> provides an abstract base class for 
Lua types (nil, boolean, number, string, function).
