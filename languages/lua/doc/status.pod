...
=item * userdata F<languages/lua/type/luauserdata.pir>

This type allows OO extension.

Currently, C<file> in I/O library is an C<userdata>.

=item * thread F<languages/lua/lib/thread.pir>
...
Lua types (userdata, thread).

F<languages/lua/pmc/luabase.pmc> provides an abstract base class for 
Lua types (nil, boolean, number, string, table, function).
