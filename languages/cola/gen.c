...
void gen_ast(AST * ast) {
	...
	for(p = ast; p; p = p->next) {
		if(p->class == AST_STATEMENT &&
			p->asttype == ASTT_CLASS_DECL) {
			gen_class_decl(p);
			printf("#End of class\n\n");
			continue;
		}
...
void gen_unary_expr(AST * p) {
	...
	else if(p->class == AST_EXPRESSION)
		gen_expr(p, p->targ);	
...
void gen_if(AST * p) {
	...
	if(!p->Attr.Conditional.condition
		|| p->Attr.Conditional.condition->class != AST_EXPRESSION) {
...
void gen_boolean(AST * p) {
	...
	/* Assume arg1->class is an AST_EXPRESSION for now */
