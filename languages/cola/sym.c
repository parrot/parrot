...
AST * new_ast(int class, int asttype, AST * arg1, AST * arg2) {
...
AST * new_statement(int stmnttype, AST * left, AST * right) {
	AST * p = new_ast(AST_STATEMENT, stmnttype, left, right);
	return p;	
}

AST * new_expression(int exprtype, AST * left, AST * right) {
	AST * p = new_ast(AST_EXPRESSION, exprtype, left, right);
	return p;	
}
...
Symbol * store_identifier(SymbolTable * tab, const char * name, int class, Symbol * type) {
	Symbol * s;
	s = new_symbol(class, name);
	s->type = type;
	s->scope = scope;
	store_symbol(tab, s);
	return s;
