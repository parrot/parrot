...
=over4
=item * type - corresponds to one of the constants in the .NET Specification
Partition II Section 22.1.15.
=item * byref - set if the byref flag is set on the type.
=item * token - for some values of type a token is also expected; it is stored
in this entry.
